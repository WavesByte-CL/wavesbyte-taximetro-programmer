...
let isProgramFinished = false; // Variable para controlar si la programación finalizó
let isReloading = false; // Nueva variable para evitar recargas múltiples
...
async function checkStatus() {
   if (isProgramFinished) return;
    ...
    try {
        ...
        if (data.status) {
            console.log("Estado actualizado desde el servidor:", data.status);
            addLogMessage(`<strong>Status:</strong> ${data.status}`);
            ...
            if (data.status === "Finalizado") {
                 ...
                 if (!isReloading) {
                    alert(
                     ...
                     isReloading = true; // Establecer el flag para evitar múltiples recargas
                     location.reload();
                 }
